...
test('catalog deps are replace', async () => {
  ...
  const catalogResolver = resolveFromCatalog.bind(null, getCatalogsFromWorkspaceManifest(workspaceManifest))
  expect(await createExportableManifest(process.cwd(), catalogProtocolPackageManifest, { catalogResolver })).toStrictEqual({
    name: 'catalog-protocol-package',
    version: '1.0.0',
    dependencies: {
      bar: '^1.2.3',
